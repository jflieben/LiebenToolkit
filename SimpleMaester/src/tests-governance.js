...
(() => {
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        ...
        if (!p1.length) return { id: 'SM.GOV.1022', status: 'Failed', reason: `No Entra ID P1 SKUs found in subscribed licenses. Found: ${skus.map(s => s.skuPartNumber).join(', ')}`, durationMs: ms(t0) };
        const total = p1.reduce((n, s) => n + (s.prepaidUnits?.enabled || 0), 0);
        return { id: 'SM.GOV.1022', status: 'Passed', reason: `${p1.length} P1 SKU(s) found with ${total} total enabled unit(s): ${p1.map(s => s.skuPartNumber).join(', ')}.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1022', e, t0); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        ...
        if (!p2.length) return { id: 'SM.GOV.1023', status: 'Failed', reason: `No Entra ID P2 SKUs found. PIM and Identity Protection features require P2. Found: ${skus.map(s => s.skuPartNumber).join(', ')}`, durationMs: ms(t0) };
        const total = p2.reduce((n, s) => n + (s.prepaidUnits?.enabled || 0), 0);
        return { id: 'SM.GOV.1023', status: 'Passed', reason: `${p2.length} P2 SKU(s) found with ${total} total enabled unit(s): ${p2.map(s => s.skuPartNumber).join(', ')}.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1023', e, t0); }
    },
  });

  // ─────────────────────────────────────────────────────────────────────────────
  // PIM POLICY RULES (SM.GOV.1085, SM.GOV.1111, SM.GOV.1112)
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        ...
        if (!rules) return { id: 'SM.GOV.1085', status: 'Skipped', reason: 'Could not retrieve PIM policy rules — requires Entra ID P2 and PIM access.', durationMs: ms(t0) };
        ...
        if (enabRule) {
          ...
          if (hasMfa) return { id: 'SM.GOV.1085', status: 'Passed', reason: 'PIM activation policy for Global Administrator requires MFA (MultiFactorAuthentication in enabledRules).', durationMs: ms(t0) };
        }
        ...
        if (authCtxRule && authCtxRule.isEnabled) {
          return { id: 'SM.GOV.1085', status: 'Passed', reason: 'PIM activation policy for Global Administrator requires authentication context (MFA/CA).', durationMs: ms(t0) };
        }
        const current = enabRule ? (enabRule.enabledRules || []).join(', ') || 'none' : 'no enablement rule found';
        return { id: 'SM.GOV.1085', status: 'Failed', reason: `PIM activation policy for Global Administrator does NOT require MFA. Enabled rules: ${current}.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1085', e, t0, 'Requires Entra ID P2 and PIM'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        const rules = await getGaActivationRules();
        if (!rules) return { id: 'SM.GOV.1111', status: 'Skipped', reason: 'Could not retrieve PIM policy rules — requires Entra ID P2 and PIM access.', durationMs: ms(t0) };
        const enablementRules = rules.filter(r => r['@odata.type'] === '#microsoft.graph.unifiedRoleManagementPolicyEnablementRule');
        if (!enablementRules.length) return { id: 'SM.GOV.1111', status: 'Skipped', reason: 'PIM activation enablement rule not found.', durationMs: ms(t0) };
        ...
        if (!hasJustification) {
          return { id: 'SM.GOV.1111', status: 'Failed', reason: `PIM activation policy for Global Administrator does NOT require justification. Enabled rules: ${enabled.join(', ') || 'none'}.`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1111', status: 'Passed', reason: 'PIM activation policy for Global Administrator requires justification.', durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1111', e, t0, 'Requires Entra ID P2 and PIM'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        const rules = await getGaActivationRules();
        if (!rules) return { id: 'SM.GOV.1112', status: 'Skipped', reason: 'Could not retrieve PIM policy rules — requires Entra ID P2 and PIM access.', durationMs: ms(t0) };
        ...
        if (!expRule) return { id: 'SM.GOV.1112', status: 'Skipped', reason: 'PIM expiration rule for activation not found.', durationMs: ms(t0) };
        if (!expRule.isExpirationRequired) {
          return { id: 'SM.GOV.1112', status: 'Failed', reason: 'PIM activation for Global Administrator has no expiration limit (isExpirationRequired=false).', durationMs: ms(t0) };
        }
        ...
        if (hours === null) return { id: 'SM.GOV.1112', status: 'Passed', reason: `PIM activation expiration is required. Duration: ${dur || 'not parsed'}.`, durationMs: ms(t0) };
        if (hours > 8) return { id: 'SM.GOV.1112', status: 'Failed', reason: `PIM activation maximum duration is ${hours}h — should be 8 hours or less. Current: ${dur}.`, durationMs: ms(t0) };
        return { id: 'SM.GOV.1112', status: 'Passed', reason: `PIM activation maximum duration is ${hours}h (${dur}) — within recommended 8-hour limit.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1112', e, t0, 'Requires Entra ID P2 and PIM'); }
    },
  });

  // ─────────────────────────────────────────────────────────────────────────────
  // FEDERATED IDENTITY CREDENTIALS ON PRIVILEGED APPS (SM.GOV.1147)
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        ...
        if (!privApps.length) return { id: 'SM.GOV.1147', status: 'Passed', reason: 'No app registrations with high-privilege application permissions found.', durationMs: ms(t0) };
        ...
        if (!findings.length) return { id: 'SM.GOV.1147', status: 'Passed', reason: `${privApps.length} privileged app(s) checked — none have federated identity credentials.`, durationMs: ms(t0) };
        const detail = findings.map(f => `${f.name} (${f.creds} federated cred(s))`).join(', ');
        return { id: 'SM.GOV.1147', status: 'Failed', reason: `${findings.length} privileged app(s) have federated identity credentials: ${detail}. Verify these are intentional and from trusted sources.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1147', e, t0); }
    },
  });

  // ─────────────────────────────────────────────────────────────────────────────
  // ACCESS REVIEWS (SM.GOV.1090, SM.GOV.1091) — requires AccessReview.Read.All
  // ─────────────────────────────────────────────────────────────────────────────
  tests.push({
    id: 'SM.GOV.1090',
    ...
    async run() {
      ...
      try {
        ...
        if (!defs.length) return { id: 'SM.GOV.1090', status: 'Failed', reason: 'No access review definitions found. Configure access reviews for privileged Entra ID roles.', durationMs: ms(t0) };
        ...
        if (!roleReviews.length) {
          return { id: 'SM.GOV.1090', status: 'Failed', reason: `${defs.length} access review(s) found but none appear to target privileged roles. Configure reviews for privileged role memberships.`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1090', status: 'Passed', reason: `${roleReviews.length} access review(s) targeting privileged roles found out of ${defs.length} total.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1090', e, t0, 'Requires AccessReview.Read.All scope'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        ...
        if (!defs.length) return { id: 'SM.GOV.1091', status: 'Failed', reason: 'No access review definitions found. Configure access reviews to remove stale privileged accounts.', durationMs: ms(t0) };
        ...
        if (!recurring.length) return { id: 'SM.GOV.1091', status: 'Failed', reason: `${defs.length} access review(s) found but none are recurring. Stale account review should be a recurring process.`, durationMs: ms(t0) };
        return { id: 'SM.GOV.1091', status: 'Passed', reason: `${recurring.length} recurring access review(s) found that help manage stale accounts.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1091', e, t0, 'Requires AccessReview.Read.All scope'); }
    },
  });

  // ─────────────────────────────────────────────────────────────────────────────
  // ENTITLEMENT MANAGEMENT (SM.GOV.1106–SM.GOV.1110) — requires EntitlementManagement.Read.All
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        ...
        if (!policies.length) return { id: 'SM.GOV.1106', status: 'Skipped', reason: 'No access package assignment policies found (Entitlement Management may not be in use).', durationMs: ms(t0) };
        const noApproval = policies.filter(p => !p.requestApprovalSettings?.isApprovalRequired);
        if (noApproval.length) {
          return { id: 'SM.GOV.1106', status: 'Failed', reason: `${noApproval.length}/${policies.length} assignment polic${noApproval.length === 1 ? 'y does' : 'ies do'} not require approval: ${noApproval.map(p => p.displayName).slice(0, 5).join(', ')}.`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1106', status: 'Passed', reason: `All ${policies.length} assignment polic${policies.length === 1 ? 'y requires' : 'ies require'} approval.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1106', e, t0, 'Requires EntitlementManagement.Read.All scope'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        const policies = await getAssignmentPolicies();
        if (!policies.length) return { id: 'SM.GOV.1107', status: 'Skipped', reason: 'No assignment policies found.', durationMs: ms(t0) };
        const noReview = policies.filter(p => !p.accessReviewSettings || !p.accessReviewSettings.isEnabled);
        if (noReview.length) {
          return { id: 'SM.GOV.1107', status: 'Failed', reason: `${noReview.length}/${policies.length} polic${noReview.length === 1 ? 'y does' : 'ies do'} not have access review enabled.`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1107', status: 'Passed', reason: `All ${policies.length} polic${policies.length === 1 ? 'y has' : 'ies have'} access review enabled.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1107', e, t0, 'Requires EntitlementManagement.Read.All scope'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        const policies = await getAssignmentPolicies();
        if (!policies.length) return { id: 'SM.GOV.1108', status: 'Skipped', reason: 'No assignment policies found.', durationMs: ms(t0) };
        ...
        if (noJustification.length) {
          return { id: 'SM.GOV.1108', status: 'Failed', reason: `${noJustification.length}/${policies.length} polic${noJustification.length === 1 ? 'y does' : 'ies do'} not require requestor justification.`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1108', status: 'Passed', reason: `All ${policies.length} polic${policies.length === 1 ? 'y requires' : 'ies require'} requestor justification.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1108', e, t0, 'Requires EntitlementManagement.Read.All scope'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        ...
        if (!orgs.length) return { id: 'SM.GOV.1109', status: 'Skipped', reason: 'No connected organizations found in Entitlement Management.', durationMs: ms(t0) };
        ...
        if (withExternal.length && !withSponsor.length) {
          return { id: 'SM.GOV.1109', status: 'Failed', reason: `${orgs.length} connected org(s) exist but no policies with external requestors require sponsor approval.`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1109', status: 'Passed', reason: `${orgs.length} connected org(s) found. External requestor policies appear to have sponsor approval configured.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1109', e, t0, 'Requires EntitlementManagement.Read.All scope'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        const policies = await getAssignmentPolicies();
        if (!policies.length) return { id: 'SM.GOV.1110', status: 'Skipped', reason: 'No assignment policies found.', durationMs: ms(t0) };
        ...
        if (selfAssignable.length) {
          return { id: 'SM.GOV.1110', status: 'Failed', reason: `${selfAssignable.length} assignment polic${selfAssignable.length === 1 ? 'y allows' : 'ies allow'} self-service access without approval: ${selfAssignable.map(p => p.displayName).slice(0, 5).join(', ')}.`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1110', status: 'Passed', reason: `None of ${policies.length} assignment polic${policies.length === 1 ? 'y allows' : 'ies allow'} self-assignment without approval.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1110', e, t0, 'Requires EntitlementManagement.Read.All scope'); }
    },
  });
  ...
  // ─────────────────────────────────────────────────────────────────────────────
  // MICROSOFT DEFENDER FOR IDENTITY (SM.GOV.1059)
        ...
        if (!found) {
          ...
          try {
            ...
            if (mdiControl) {
              ...
              return { id: 'SM.GOV.1059', status: score100 > 50 ? 'Passed' : 'Failed', reason: `MDI found via Secure Score. Control: ${mdiControl.controlName}, score: ${mdiControl.score}/${mdiControl.total}.`, durationMs: ms(t0) };
            }
          } catch {}
          return { id: 'SM.GOV.1059', status: 'Failed', reason: 'Microsoft Defender for Identity service principal not found. MDI does not appear to be provisioned in this tenant.', durationMs: ms(t0) };
        }
        const enabled = found.accountEnabled !== false;
        if (!enabled) return { id: 'SM.GOV.1059', status: 'Failed', reason: `MDI service principal found (${found.displayName}) but is disabled.`, durationMs: ms(t0) };
        return { id: 'SM.GOV.1059', status: 'Passed', reason: `Microsoft Defender for Identity is provisioned: ${found.displayName} (appId: ${found.appId}).`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1059', e, t0); }
    },
  });

  // ─────────────────────────────────────────────────────────────────────────────
  // AZURE RBAC (SM.GOV.1056, SM.GOV.1064, SM.GOV.1065) — requires management.azure.com token
  // ─────────────────────────────────────────────────────────────────────────────
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        ...
        if (!subs.length) return { id: 'SM.GOV.1056', status: 'Skipped', reason: 'No Azure subscriptions found or accessible.', durationMs: ms(t0) };
        ...
        if (!totalEligible && totalPermanent > 0) {
          return { id: 'SM.GOV.1056', status: 'Failed', reason: `No PIM-eligible Azure role assignments found, but ${totalPermanent} permanent Owner/Contributor assignments exist. Use PIM for privileged Azure roles. ${subDetails.join('; ')}`, durationMs: ms(t0) };
        }
        ...
        return { id: 'SM.GOV.1056', status, reason: msg, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1056', e, t0, 'Requires Azure subscription reader access (management.azure.com)'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        const subs = await listSubscriptions();
        if (!subs.length) return { id: 'SM.GOV.1064', status: 'Skipped', reason: 'No Azure subscriptions found or accessible.', durationMs: ms(t0) };
        const OWNER_ID = '8e3af657-a8ff-443c-a75c-2fe8c4bcb635';
        ...
        if (ownerAssignments.length) {
          return { id: 'SM.GOV.1064', status: 'Failed', reason: `${ownerAssignments.length} direct Owner role assignment(s) on subscriptions. Use PIM eligible assignments instead. Affected subscriptions: ${[...new Set(ownerAssignments.map(o => o.sub))].join(', ')}.`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1064', status: 'Passed', reason: `No direct Owner role assignments found on ${subs.length} subscription(s). Owner access appears to be managed via PIM or no assignments exist.`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1064', e, t0, 'Requires Azure subscription reader access'); }
    },
  });
  ...
  tests.push({
    ...
    async run() {
      ...
      try {
        const subs = await listSubscriptions();
        if (!subs.length) return { id: 'SM.GOV.1065', status: 'Skipped', reason: 'No Azure subscriptions found or accessible.', durationMs: ms(t0) };
        const classicAdmins = [];
        for (const sub of subs.slice(0, 10)) {
          try {
        ...
        if (classicAdmins.length) {
          ...
          return { id: 'SM.GOV.1065', status: 'Failed', reason: `${classicAdmins.length} classic administrator(s) found — these roles are deprecated: ${detail}`, durationMs: ms(t0) };
        }
        return { id: 'SM.GOV.1065', status: 'Passed', reason: `No classic administrators (Co-Admin, Service Admin) found across ${subs.length} subscription(s).`, durationMs: ms(t0) };
      } catch (e) { return skipOn('SM.GOV.1065', e, t0, 'Requires Azure subscription reader access'); }
    },
  });
